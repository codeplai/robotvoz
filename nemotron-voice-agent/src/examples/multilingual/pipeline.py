...
from examples.multilingual.llm_nube import FiltroEtiquetasModelo, NvidiaLLMNube
from examples.multilingual.stt_reciclado import NvidiaSTTServiceReciclado
...
async def bot(runner_args: RunnerArguments) -> None:
    ...
    # Servicio por defecto = el que fija examples_registry.yaml para este ejemplo.
    # El original pedia load_service_entry(cat, ""), que devuelve la PRIMERA
    # entrada del catalogo: en la version local coincidia con qwen35-4b por
    # casualidad, pero en la nube cogia nemotron-lightning (el primero de
    # services.cloud.yaml) aunque el registro dijera nemotron-super. Solo afecta
    # a clientes que no eligen servicio (el movil y robot_voz.py); si el cliente
    # manda llm_id/asr_id/tts_id, el cuerpo ya viene hidratado con sus datos.
    defaults_registro = examples_registry.find(body.get("pipeline_mode", ""))["defaults"]

    def _entrada_por_defecto(categoria: str) -> dict:
        ids = defaults_registro.get(categoria) or []
        return load_service_entry(categoria, ids[0] if isinstance(ids, list) and ids else "")

    default_llm = _entrada_por_defecto("llm")
    default_tts = _entrada_por_defecto("tts")
    default_asr = _entrada_por_defecto("asr")
    ...
    # Recicla el stream gRPC cada ~20 s de audio: con el micro abierto de forma
    # continua, el NIM deja de consumir audio tras ~30 s en el mismo stream, sin
    # error. Ver stt_reciclado.py.
    stt = NvidiaSTTServiceReciclado(**asr_kwargs, stop_history=400)
    logger.info(
    ...
    # Reintenta errores transitorios de la API y limpia etiquetas de plantilla
    # que el modelo escribe como texto. Ver llm_nube.py.
    llm = NvidiaLLMNube(
        api_key=os.getenv("NVIDIA_API_KEY"),
        base_url=base_url,
        settings=llm_settings,
        # Solo la version nube lo define (docker-compose.override.yml).
        modelo_respaldo=os.getenv("LLM_MODELO_RESPALDO", "").strip() or None,
    )

    # --- Skills del robot -------------------------------------------------
    # El esquema lleva la UNION de herramientas de todos los skills, porque no
    # se puede cambiar a mitad de sesion. El permiso por skill lo aplican los
    # manejadores. El estado es por sesion: dos conversaciones no se pisan.
    estado_skill = EstadoSkill(
        prompt_key if prompt_key in SKILLS.values() else next(iter(SKILLS.values())),
        load_prompt_catalog(__file__),
    )
    manejadores = construir_manejadores(estado_skill)
    tools_schema, herramientas_registradas = build_tools_schema(
        __file__, herramientas_de_todos_los_skills(__file__), manejadores
    )
    ...
    tts_kwargs: dict = {
        ...
        "use_ssl": tts_ssl,
        "text_filters": [FiltroEtiquetasModelo(), NemotronSpeechTextFilter()],
    ...
    # tool_choice="auto": el modelo decide cuando invocar una herramienta.
    # Sin tools_schema el contexto se crea igual, solo que sin herramientas.
    if tools_schema is not None:
    ...
    # El cambio de skill por voz tiene que reescribir ESTE contexto compartido.
    # La herramienta recibe en params.context una COPIA: PerTurnReminderProcessor
    # reenvia un LLMContext nuevo en cada turno. Reescribir esa copia no tenia
    # efecto y el modelo seguia recibiendo el prompt del skill inicial.
    # renderizar() vuelve a añadir el bloque de idioma fijo de la sesion, que se
    # perdia al sustituir el prompt por el contenido crudo del skill.
